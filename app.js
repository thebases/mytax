...
function detectType(code) {
  if (!code) return { label: "Không xác định", bg: "#f4f4f5", bd: "#e4e4e7" };
  if (/^\d{13}$/.test(code))
...
function normalizeData(raw, iemail, iphone) {
  ...
  if (!d) return null;
  const taxId =
    d.id || d.tax_id || d.taxId || d.mst || d.tax || d.code || d.MST || null;
  const name =
  ...
  const phone =
    d.phone || d.sdt || d.so_dien_thoai || d.contact_phone || iphone || null;
...
async function fetchAndUpsert(code, iemail, iphone) {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error("Không lấy được dữ liệu (HTTP " + res.status + ").");
    const json = await res.json();
    const norm = normalizeData(json, iemail, iphone);
    if (!norm || !norm.taxId || !norm.name) {
      setStatus("Không tìm thấy dữ liệu cho mã số thuế này.", "error");
